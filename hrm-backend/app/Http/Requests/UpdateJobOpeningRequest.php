...
class UpdateJobOpeningRequest extends FormRequest
{
    ...
    {
        $jobOpening = $this->route('jobOpening') ?? $this->route('id');
        $jobOpeningId = is_object($jobOpening) ? $jobOpening->id : $jobOpening;

        return [
            'title' => ['required', 'string', 'max:255'],
            'job_code' => ['nullable', 'string', 'max:50', Rule::unique('job_openings', 'job_code')->ignore($jobOpeningId)],
